...
{-| -}
withUnit : Maybe String -> Config msg -> Config msg
withUnit unit { mandatory, optional } =
    { mandatory = mandatory, optional = { optional | unit = unit } }
...
view { mandatory, optional } =
    let
        ...
        defaultInputAttrs =
            [ Attr.class "fr-input !rounded-tl !rounded-tr-none"
            , Attr.classList
    ...
    <|
        ...
        , Html.div [ Attr.class "flex items-center" ]
            ...
            , Html.Extra.viewMaybe
                (\unit ->
                    Html.span [ Attr.class "text-nowrap fr-px-4v fr-py-2v leading-6 shadow-input-border rounded-tr bg-[var(--artwork-background-grey)]" ]
                        [ Html.text unit
                        ]
                )
                optional.unit
